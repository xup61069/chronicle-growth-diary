...
> **狀態：** 設計完成，尚未實作。本文不代表目前任何匯入路徑已有跨來源去重。
...
| 3 | Canvas 本機 pHash／dHash + Hamming threshold | 僅使用者明確選取的影像預覽 | 只標示「視覺近似」；不得認定為同一張、不得自動刪除。 |
