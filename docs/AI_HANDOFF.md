...
4. **協作能力**：Issue #12 的家庭留言第一階段已完成：只限 private event 與 active member，作者可 tombstone 自己的留言、owner 可 moderation，已刪除正文不再投影，稽核不含正文，public／link、AI 與 portable archive 均不含留言。即時共編仍屬未實作高風險項目，需另行設計 presence、衝突、離線與保留策略。
