...
| 範圍 | 狀態 | 可重現方法 |
| --- | --- | --- |
| 格式、型別、單元測試與正式建置 | 通過 | `pnpm lint && pnpm check && pnpm test`，再分別執行 `./node_modules/.bin/vite build` 與 `pnpm exec esbuild server/_core/index.ts --platform=node --packages=external --bundle --format=esm --outdir=dist`。目前基準為 **68 個測試檔、190 項 Vitest**。 |
...
| 每日回憶檢查（A：不外送） | 通過 | `growth_diary_recall_preferences` 預設 `enabled: false`，只保存 owner 偏好、時區、Heartbeat task UID、最後檢查時間、兩個計數與狀態；不保存標題、正文、媒體、地點、收件地址或通知內容。來源查詢只投影事件日期、精度、解鎖時間與 private 範圍；`server/scheduled/recallCheck.ts` 只接受 Heartbeat cron task UID，回應不含事件數或日記欄位。單元測試覆蓋 private/public/link、日期精度、到期／未到期信件、owner 偏好啟停、cron-only、停止／孤兒 no-op 與錯誤回應；HTTPS local-auth 桌面與 375px 回歸確認預設關閉、手動檢查停用、無 Email／推播／內容外送說明，以及未發布時拒絕建立排程。正式發布後才可實際建立與測試每日 task；本階段沒有 Email 或 Web Push 遞送。 |
