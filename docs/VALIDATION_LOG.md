...
| 公開首頁 375px | 通過 | 設定 HTTPS `CHRONICLE_E2E_BASE_URL` 後執行 `pnpm test:e2e:mobile-nav`。覆蓋跳至主要內容、行動導覽、範例入口、時間帶鍵盤探索、事件類型篩選的 `aria-pressed`、僅公開欄位的關鍵字搜尋、自動完成的 Escape／Arrow／Enter 選取、搜尋命中 `mark`、日期與類型交集篩選、由新到舊／由舊到新排序、零結果插圖、近期事件建議、零結果清除、減少動態下無結果過渡與離線紀錄入口。首頁改採事件、日期、資料可見範圍與操作流程等直接產品文案，已以桌面與 375px 檢查可讀性。 |
