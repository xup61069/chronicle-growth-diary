...
| 正式首頁空白修復與 PWA 升級 | 通過 | 正式網域曾因 Recharts 被手動抽出 `charts` chunk，於 React runtime export 初始化前存取 `forwardRef` 而使入口 module 評估失敗、root 空白；已移除該分包，並保留 Recharts 在延遲載入的 GrowthDashboard 路由。`viteCodeSplitting.test.ts` 防止 charts chunk 回歸，`pwaCacheUpgrade.test.ts` 保護 `skipWaiting`、`clientsClaim` 與過期 precache 清理。正式 `chronotime-w3ztsoiq.manus.space` 已以匿名桌面瀏覽器確認首頁內容、登入按鈕與搜尋控制可見，並以 `pnpm test:e2e:mobile-nav` 對同一正式網域確認 375px 導覽、搜尋、篩選、URL 清除、零結果與鍵盤回歸通過。 |
