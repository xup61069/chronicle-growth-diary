...
try {
  ...
  if (viewport.width > 375) {
    ...
    const secondJourneyDraft = journeyImport.getByTestId('journey-draft-isolated-journey-entry-second');
    ...
    const secondJourneyTitleInput = journeyImport.getByRole('textbox', { name: 'Journey 草稿標題 isolated-journey-entry-second', exact: true });
    await secondJourneyTitleInput.fill('Journey 隔離驗證記事');
    await secondJourneyDateInput.fill('2026-08-22T09:00');
    await journeyImport.getByRole('button', { name: '以本機短標題／UTC 日期檢查' }).click();
    await journeyImport.getByText('相同短標題與 UTC 日期候選', { exact: true }).waitFor({ timeout: 10_000 });
    assert(journeyImportRequestCount === 0, '短標題／UTC 日期候選只可在本機審核，不得於確認前建立事件。');
    await journeyImport.getByRole('button', { name: '略過後續 1 筆' }).click();
    assert(!(await secondJourneyDraft.locator('input[type="checkbox"]').isChecked()), '略過候選只應取消目前第二筆草稿的選取，且可由保留全部復原。');
