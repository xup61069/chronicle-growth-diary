...
import {
  ...
  getFullDiaryArchive,
  getDiaryAuditLogs,
  getFamilyMilestoneAudienceAudit,
  getDiaryMembers,
  getFamilyMilestones,
  getEventComments,
  getEventReactions,
...
export const diaryRouter = router({
  ...
  deleteEventComment: protectedProcedure.input(z.object({ eventId: z.number().int().positive(), commentId: z.number().int().positive() })).mutation(({ ctx, input }) => deleteEventComment(ctx.user.id, input.eventId, input.commentId)),
