...
async function getOrCreateDiary(userId: number) {
  ...
  const created = await db
    .select()
    .from(growthDiaries)
    .where(eq(growthDiaries.userId, userId))
    .orderBy(desc(growthDiaries.id))
    .limit(1);
...
async function assertEventOwnership(eventId: number, userId: number) {
  ...
  const event = await db
    .select({ id: growthEvents.id, diaryId: growthEvents.diaryId })
    .from(growthEvents)
...
async function saveEventTags(eventId: number, userId: number, rawTagNames: string[]) {
  ...
  for (const name of tagNames) {
    const existing = await db
      .select()
      .from(growthTags)
      .where(and(eq(growthTags.userId, userId), eq(growthTags.name, name)))
      .limit(1);
...
export async function getDiarySnapshot(userId: number) {
  ...
  const events = await db
    .select()
    .from(growthEvents)
    .where(eq(growthEvents.diaryId, diary.id))
    .orderBy(asc(growthEvents.occurredAt), asc(growthEvents.id));
  const tags = await db
    .select()
    .from(growthTags)
    .where(eq(growthTags.userId, userId))
    .orderBy(asc(growthTags.name));
  const eventIds = events.map((event) => event.id);

  const taggedRows = eventIds.length
    ? await db
      .select({ eventId: growthEventTags.eventId, id: growthTags.id, name: growthTags.name, color: growthTags.color })
      .from(growthEventTags)
      .innerJoin(growthTags, eq(growthEventTags.tagId, growthTags.id))
      .where(inArray(growthEventTags.eventId, eventIds))
    : [];
  const mediaRows = eventIds.length
    ? await db
      .select()
      .from(growthEventMedia)
      .where(inArray(growthEventMedia.eventId, eventIds))
      .orderBy(asc(growthEventMedia.sortOrder), asc(growthEventMedia.id))
    : [];
...
export async function uploadDiaryEventImage(input: {
  userId: number;
  eventId: number;
  fileName: string;
  mimeType: string;
  ...
  const stored = await storagePut(
    `growth-diary/${input.userId}/event-${input.eventId}/${fileName}`,
    bytes,
    input.mimeType,
  );
  const existingMedia = await db
    .select({ count: growthEventMedia.id })
    .from(growthEventMedia)
    .where(eq(growthEventMedia.eventId, input.eventId));

  await db.insert(growthEventMedia).values({
...
export async function deleteDiaryEventMedia(userId: number, mediaId: number) {
  ...
  const media = await db
    .select({ id: growthEventMedia.id })
    .from(growthEventMedia)
    .innerJoin(growthEvents, eq(growthEventMedia.eventId, growthEvents.id))
    .innerJoin(growthDiaries, eq(growthEvents.diaryId, growthDiaries.id))
    .where(and(eq(growthEventMedia.id, mediaId), eq(growthDiaries.userId, userId)))
    .limit(1);
