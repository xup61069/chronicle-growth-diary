...
describe("recall check UI privacy contract", () => {
  it("keeps the owner-only daily check default-off and states the no-delivery boundary", () => {
    expect(editor).toContain("DAILY RECALL CHECK / OWNER ONLY");
    expect(editor).toContain("每天自動檢查");
    expect(editor).toContain("預設關閉");
    expect(editor).toContain("不寄送 Email、不推播、不保存日記內容、標題、照片或地點");
