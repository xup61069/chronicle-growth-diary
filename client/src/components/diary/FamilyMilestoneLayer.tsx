...
type FamilyMilestoneLayerProps = {
  ...
  audienceAuditRange: FamilyAudienceAuditDateRange;
  audienceAuditRangeError: string | null;
  onAudienceAuditRangeChange: (range: FamilyAudienceAuditDateRange) => void;
...
export function FamilyMilestoneLayer({ milestones, canManage, sourceEvents, familyMembers, audienceAudit, audienceAuditRange, audienceAuditRangeError, onAudienceAuditRangeChange, isLoadingAudienceAudit, isSaving, onCreate, onUpdate, onDelete }: FamilyMilestoneLayerProps) {
  ...
  return <section className="family-milestone-layer" aria-labelledby="family-milestone-title">
    ...
    {canManage ? <details className="family-audience-audit"><summary><History size={14} />查看已確認的受眾異動 <small>僅顯示時間、動作與大事記識別碼；不含摘要或成員資料。</small></summary><p>這是 owner 的被動稽核檢視。開啟不會通知任何成員，也不會建立新的紀錄。</p><div className="family-audience-audit-filter"><label>開始日期<input aria-label="受眾稽核開始日期" type="date" value={audienceAuditRange.fromDate} onChange={(event) => onAudienceAuditRangeChange({ ...audienceAuditRange, fromDate: event.target.value })} /></label><label>結束日期<input aria-label="受眾稽核結束日期" type="date" value={audienceAuditRange.toDate} onChange={(event) => onAudienceAuditRangeChange({ ...audienceAuditRange, toDate: event.target.value })} /></label><button type="button" onClick={() => onAudienceAuditRangeChange({ fromDate: "", toDate: "" })} disabled={!audienceAuditRange.fromDate && !audienceAuditRange.toDate}>清除日期</button></div>{audienceAuditRangeError ? <p className="family-audience-audit-error" role="alert">{audienceAuditRangeError}</p> : null}{isLoadingAudienceAudit ? <span className="family-audience-audit-loading"><Loader2 size={13} className="animate-spin" /> 讀取中</span> : audienceAudit.length ? <div>{audienceAudit.map((audit) => <article key={audit.id}><time>{new Date(audit.createdAt).toLocaleString("zh-TW")}</time><b>已確認受眾範圍變更</b><small>大事記 #{audit.targetId ?? "—"}</small></article>)}</div> : <em>{audienceAuditRange.fromDate || audienceAuditRange.toDate ? "此日期區間沒有已確認的受眾範圍變更。" : "尚未有已確認的受眾範圍變更。"}</em>}</details> : null}
  </section>;
}
