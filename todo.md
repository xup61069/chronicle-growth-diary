...
- [x] 漸進拆分 `DiaryEditor.tsx` 第二階段：已抽出 `PrivateVoiceDiary` 展示與本機草稿狀態區塊，父頁仍管理錄音、上傳、轉寫與私人媒體資料寫入；通過 server-render、全量品質閘門，隔離桌面／375px 回歸均覆蓋本機草稿、預設未同意、同意後上傳入口啟用及刪除 callback，測試資料已清理且未呼叫外部轉寫。
