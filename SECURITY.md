...
| 分享與家庭協作 | 僅明確標記可分享的事件可離開私有邊界；密碼、invite 與 link token 不得以明文保存。family-only 大事記只能投影 owner 手動寫入的日期、標題與短摘要；新項目預設指定 accepted member，受眾關聯只保存 membership ID。既有項目變更有效受眾或政策時，owner 必須先在本機檢視目前／提議／加入／移除差異並第二次確認；名冊或選擇更新時須使預覽失效。成員不得讀取來源事件、正文、媒體、語音、GPS、AI 輸出、其他成員或未獲授權項目的存在性。membership 移除須 cascade 清除指定關聯，重新接受不得恢復舊授權。 | public／link／private 範圍、到期、密碼和邀請 token，以及 owner／selected／unselected／removed／reaccepted member 的大事記存取測試；受眾預覽前零 update、二次確認後單次 update、名冊更新失效。 |
