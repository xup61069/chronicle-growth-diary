...
| 分享與家庭協作 | 僅明確標記可分享的事件可離開私有邊界；密碼、invite 與 link token 不得以明文保存。family-only 大事記只能投影 owner 手動寫入的日期、標題與短摘要；成員不得讀取來源事件、正文、媒體、語音、GPS 或 AI 輸出。 | public／link／private 範圍、到期、密碼和邀請 token，以及 owner／accepted／removed member 的大事記存取測試。 |
