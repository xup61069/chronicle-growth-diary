...
| 分享與家庭協作 | 僅明確標記可分享的事件可離開私有邊界；密碼、invite 與 link token 不得以明文保存。family-only 大事記只能投影 owner 手動寫入的日期、標題與短摘要；新項目預設指定 accepted member，受眾關聯只保存 membership ID。成員不得讀取來源事件、正文、媒體、語音、GPS、AI 輸出、其他成員或未獲授權項目的存在性。membership 移除須 cascade 清除指定關聯，重新接受不得恢復舊授權。 | public／link／private 範圍、到期、密碼和邀請 token，以及 owner／selected／unselected／removed／reaccepted member 的大事記存取測試。 |
...
| 匯入／匯出 | Day One JSON／ZIP 僅在使用者選檔後由瀏覽器讀取，先建立暫態草稿、再由明確確認寫入 private 事件；只保留日期、純文字與受限標籤，捨棄媒體、位置、裝置、天氣、rich text、來源 URL、ZIP 與來源 UUID。匯出不得包含 session、憑證、密碼雜湊、分享 token、存取紀錄或私有 storage key。全量 ZIP 還原必須先在瀏覽器驗證 manifest／payload／附件 checksum，再由 owner-only staging 重新核對大小與 SHA-256；僅 typed confirmation 的單一交易可覆寫日記，且結果強制 private。 | Day One archive path、欄位剝除、大小／筆數、未確認 no-op 與 private payload；portable、frontmatter、media archive、`fullDiaryArchive.test.ts`、`archiveRestore.test.ts`、全量封存 security contract 與 owner-only E2E。 |
